...
[Route("[controller]")]
public class AthleteController : JiuJitsuControllerBase
{
    [AllowAnonymous]
    [HttpPost("login")]
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var result = await athleteService.LoginAsync(requestModel.Email, requestModel.Password, cancellationToken);
            ...
            var athlete = result.Value;
            var accessToken = tokenService.GenerateAccessToken(athlete, Environment.GetEnvironmentVariable("SECRET_ACCESS_TOKEN"));
       ...
       CancellationToken cancellationToken)
    {
        ...
        try
        {
            var result = await athleteService.GetByRefreshTokenAsync(refreshToken, cancellationToken);
